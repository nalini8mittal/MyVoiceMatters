...
def naive_bayes(test):
	...
	for i in dataset.keys():
		# Category Probability: we have 2 categories, (1 and -1), we access using dictionary
...
while(True):
        print ('Enter Tweet:')
        text=input()                    
        #put limit on input tweet(must be less than 140 characters
        if len(text) > 140:
                print("Tweet charater limit is 140")
        else:
                break;
...
if result['1'] > result['-1']:
	print ('Non-Complaint/No obvious sentiment')
else:
	print ('Complaint/Grievance')
print(result['1'])
print(result['-1'])
